/* CDP screenshot helper with scroll-to-selector. Usage:
   node _cdp-shot-at.mjs <outPng> <width> <height> <waitMs> <selector> [mobileDPR] */
import { spawn } from 'node:child_process';
import { writeFileSync, mkdtempSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';

const [, , outPng, wStr, hStr, waitStr, selector, dprStr] = process.argv;
...
if (selector) {
  // scroll to the selector and wait for reveals
  await send('Runtime.evaluate', {
    expression: `(()=>{const el=document.querySelector(${JSON.stringify(selector)});if(el){el.scrollIntoView({block:'start',behavior:'instant'});}})();`,
  });
  await sleep(1200);
}
