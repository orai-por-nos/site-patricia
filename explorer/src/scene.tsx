...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<768?1.5:2));renderer.setClearColor('#f1e9db');renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;el.appendChild(renderer.domElement);
  renderer.domElement.setAttribute('aria-label','Corpo humano 3D. Arraste para girar e toque para selecionar. Use as setas para girar, mais e menos para zoom, e Home para restaurar.');
  ...
  const keyboard=(event:KeyboardEvent)=>{
   ...
   event.preventDefault();const offset=camera.position.clone().sub(controls.target);
   ...
   if(event.key==='ArrowUp')spherical.phi=Math.max(.05,spherical.phi-.12);
   if(event.key==='ArrowDown')spherical.phi=Math.min(Math.PI-.05,spherical.phi+.12);
   if(event.key==='+'||event.key==='=')spherical.radius=Math.max(.07,spherical.radius*.9);
   if(event.key==='-')spherical.radius=Math.min(40,spherical.radius*1.1);
   camera.position.copy(controls.target).add(new T.Vector3().setFromSpherical(spherical));
   if(event.key==='Home')fit(latest.current.view,amount);
  ...
  const hover=document.createElement('div');hover.className='hbe-hover';hover.setAttribute('role','tooltip');hover.hidden=true;el.appendChild(hover);
  ...
  const fit=(view:string,extent=0)=>{
   ...
   const normalDistance=Math.max(2.6,1.95/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*Math.max(1,.7/camera.aspect));
   const atlasDistance=Math.max(packingHeight,packingWidth/camera.aspect)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.15;
  ...
  let inViewport=true;
  const visibilityObserver=new IntersectionObserver(entries=>{inViewport=entries[0].isIntersecting;if(inViewport)dirty=true;});visibilityObserver.observe(el);
  const clock=new T.Clock();let lastExtent=-1;
  const animate=()=>{
   if(disposed)return;frame=requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.05),s=latest.current;if(!inViewport||document.hidden)return;
   ...
   if(isolateKey!==lastIsolate||(s.isolate&&moving)){
    if(s.isolate){const box=new T.Box3();atlas.parts.forEach((p,i)=>{if(s.selected.includes(p.id))box.union(bounds[i].clone().translate(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])));});
     if(!box.isEmpty()){const center=box.getCenter(new T.Vector3()),size=box.getSize(new T.Vector3());const w=el.clientWidth,h=el.clientHeight;const availableWidth=w*.88,availableHeight=h*.88;camera.clearViewOffset();const distance=Math.max(.07,Math.max(size.y*h/availableHeight,size.x*w/availableWidth/camera.aspect,size.z)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.35);controls.maxDistance=Math.max(40,distance*2);controls.target.copy(center);camera.position.copy(center).add(new T.Vector3(.2,.1,1).normalize().multiplyScalar(distance));controls.update();dirty=true;}
    }else if(lastIsolate){camera.clearViewOffset();fit(s.view,amount);}
    lastIsolate=isolateKey;
   }
   controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;markers.visible=amount>.75;controls.autoRotate=s.rotate&&!reducedMotion.matches&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
  ...
  return()=>{disposed=true;abort.abort();cancelAnimationFrame(frame);observer.disconnect();visibilityObserver.disconnect();renderer.domElement.removeEventListener('keydown',keyboard);controls.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();};
 },[atlas]);
 return <div className="hbe-scene" ref={host}/>;
}
